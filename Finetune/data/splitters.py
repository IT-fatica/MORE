...
def random_scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
    ...
    # scaffold_sets_ = [np.array(scaffold, dtype=object) for scaffold in scaffolds.values()]
    # max_length = max(len(scaffold) for scaffold in scaffold_sets_)
    # scaffold_sets_padded = [np.pad(scaffold, (0, max_length - len(scaffold)), mode='constant', constant_values=null_value) for scaffold in scaffold_sets_]

    # # Randomize scaffold_sets
    # rng = np.random.RandomState(seed)
    # scaffold_sets = rng.permutation(scaffold_sets_padded)
    # ########## GPT ##########
...
def scaffold_split_abs_value_balance(dataset, smiles_list, task_idx=None, null_value=0, number_train=100, frac_valid=0.1,
                                     frac_test=0.1, return_smiles=False):
    if task_idx is not None:
        y_task = np.array([data.y[task_idx].item() for data in dataset])
        non_null = y_task != null_value
        smiles_list = list(compress(enumerate(smiles_list), non_null))
        y_task = y_task[non_null]
    else:
        non_null = np.ones(len(dataset)) == 1
        smiles_list = list(compress(enumerate(smiles_list), non_null))
    ...
    for scaffold_set in all_scaffold_sets:
        if len(train_idx) + len(scaffold_set) > train_cutoff:
            ...
            for idx in scaffold_set:
                if y_task[idx].item() == 1:
                    train_pos_idx.append(idx)
                else:
                    train_neg_idx.append(idx)
                train_idx.append(idx)
                if len(train_pos_idx) >= train_number // 2 and len(train_neg_idx) >= train_number // 2:
                    break
            if len(train_pos_idx) >= train_number // 2 and len(train_neg_idx) >= train_number // 2:
                break
