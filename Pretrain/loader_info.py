...
def mol_to_graph_data_obj_simple(mol):
    ...
    if x.shape[0] < 3:
        print("{} is removing".format(Chem.MolToSmiles(mol)))
        return None
    ...
    ## Descriptors
    descriptors = [[applyFunc(name, mol)] for name, _ in des_list]
    descriptors = torch.tensor(np.array(descriptors), dtype=torch.float64).T
    
    ## MACCS
    maccs_func = MACCSkeys.GenMACCSKeys(mol)
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            input_df = pd.read_csv('dataset_info/zinc_2m_MD/raw/smiles.csv')
            smiles_list = list(input_df['smiles'])
            for i in range(len(smiles_list)):
                s = smiles_list[i]
            ...
            'dataset_info/bace',
            'dataset_info/bbbp',
            'dataset_info/clintox',
            'dataset_info/esol',
            'dataset_info/freesolv',
            'dataset_info/hiv',
            'dataset_info/lipophilicity',
            'dataset_info/muv',
            # 'dataset_info/pcba/processed/smiles.csv',
            'dataset_info/sider',
            'dataset_info/tox21',
            'dataset_info/toxcast'
            ]
        ...
        # elif self.dataset == ''
            ...
            # smiles_path = 'dataset_info/mutag/raw/mutag_188_data.can'
            labels_path = os.path.join(self.root, 'raw', 'mutag_188_target.txt')
            # labels_path = 'dataset_info/mutag/raw/mutag_188_target.txt'
...
def merge_dataset_objs(dataset_1, dataset_2):
    ...
    new_dataset = MoleculeDataset(root='dataset_info/chembl_with_labels',
...
def _load_clintox_dataset(input_path):
    ...
    return preprocessed_smiles_list, preprocessed_rdkit_mol_objs_list, \
           labels.values
# input_path = 'dataset_info/clintox/raw/clintox.csv'
...
def _load_esol_dataset(input_path):
    ...
    assert len(smiles_list) == len(labels)
    return smiles_list, rdkit_mol_objs_list, labels.values
# input_path = 'dataset_info/esol/raw/delaney-processed.csv'
...
# root_path = 'dataset_info/chembl_with_labels'
...
def create_all_datasets():
    ...
    for dataset_name in downstream_dir:
        print(dataset_name)
        root = "dataset_info/" + dataset_name
        os.makedirs(root + "/processed", exist_ok=True)
        dataset = MoleculeDataset(root, dataset=dataset_name)
        print(dataset)


    dataset = MoleculeDataset(root = "dataset_info/chembl_filtered", dataset="chembl_filtered")
    print(dataset)
    dataset = MoleculeDataset(root = "dataset_info/zinc_standard_agent", dataset="zinc_standard_agent")
    print(dataset)
