...
des_list = [(name, np.float64) for name, func in sorted(Descriptors.descList) if name not in des_del] 
def applyFunc(name, m):
    ## code from KPGT (https://github.com/lihan97/KPGT/blob/390f29529dde268fed19203e7435307ae15dc083/src/data/descriptors/rdDescriptors.py#L33)
    try:
        return FUNCS[name](m)
    except:
        print("{} Can not descriptor".format(Chem.MolToSmiles(m)))
        return None
...
def mol_to_graph_data_obj_simple(mol):
    ...
    mol_prop = Chem.RemoveHs(mol, implicitOnly=True) ## to make except conformer
    ...
    ## invaild feature remove (addition)
    if x.shape[0] < 3:
        print("{} is removing".format(smiles))
        return None
    ...
    ## Descriptors
    descriptors = [[applyFunc(name, mol_prop)] for name, _ in des_list]
    descriptors = torch.tensor(np.array(descriptors), dtype=torch.float64).T ## 
    
    ## MACCS
    maccs_func = MACCSkeys.GenMACCSKeys(mol_prop)
    maccs = torch.tensor(maccs_func.ToList(), dtype=torch.long)[1:].unsqueeze(0) ## The value of the first bit of MACCS is not required.
    
    ## conformer    
    AllChem.EmbedMultipleConfs(mol_conf, numConfs=5, randomSeed=22, numThreads=0, useRandomCoords=True) ## randomly generate five confoermers using the random coordinate generation method
    li = AllChem.MMFFOptimizeMoleculeConfs(mol_conf, maxIters=2000) ## optimized to their energy-minimized forms using MMFF function
    li = [t[1] for t in li]
    sortidx = torch.argsort(torch.tensor(li))
    
    # save three with the lowest energies
    min1pos = torch.tensor(mol_conf.GetConformer(int(sortidx[0])).GetPositions())
    min2pos = torch.tensor(mol_conf.GetConformer(int(sortidx[1])).GetPositions())
    min3pos = torch.tensor(mol_conf.GetConformer(int(sortidx[2])).GetPositions())

    data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, smiles=smiles, 
                descriptors=descriptors, descriptors_ss=descriptors, maccs=maccs,
                esomeprazole=mol_conf, min1pos=min1pos, min2pos=min2pos, min3pos=min3pos
                )
...
class MoleculeDataset(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        for key in self.data.keys():
            if key == 'smiles' or key == 'esomeprazole': continue
            item, slices = self.data[key], self.slices[key]
            s = list(repeat(slice(None), item.dim()))
            s[data.__cat_dim__(key, item)] = slice(slices[idx],
                                                    slices[idx + 1])
            data[key] = item[s]
        data['smiles'] = self.data['smiles'][idx]
        try:
            data['esomeprazole'] = self.data['esomeprazole'][idx]
        except:
            pass
        return data
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
        ...
        elif self.dataset == 'zinc_2m_MD': ## 1,974,507 samples (maccs and descriptors preprocessed, with conformer not generated excluded)
            input_df = pd.read_csv('dataset_conf/zinc_2m_MD/raw/smiles.csv')
            smiles_list = list(input_df['smiles'])
            for i in range(len(smiles_list)):
                s = smiles_list[i]
                # eacch example contains a single species
                try:
                    rdkit_mol = AllChem.MolFromSmiles(s)
                    if rdkit_mol != None: # ignore invalid mol objects
                        data = mol_to_graph_data_obj_simple(rdkit_mol)
            ...
            'dataset_conf/bace',
            'dataset_conf/bbbp',
            'dataset_conf/clintox',
            'dataset_conf/esol',
            'dataset_conf/freesolv',
            'dataset_conf/hiv',
            'dataset_conf/lipophilicity',
            'dataset_conf/muv',
            # 'dataset_conf/pcba/processed/smiles.csv',
            'dataset_conf/sider',
            'dataset_conf/tox21',
            'dataset_conf/toxcast'
            ]
        ...
        # elif self.dataset == ''
            ...
            # smiles_path = 'dataset_conf/mutag/raw/mutag_188_data.can'
            labels_path = os.path.join(self.root, 'raw', 'mutag_188_target.txt')
            # labels_path = 'dataset_conf/mutag/raw/mutag_188_target.txt'
        ...
        scaler_ss.fit(data['descriptors_ss'])
        ss_norm = scaler_ss.transform(data['descriptors_ss'])
        data['descriptors_ss'] = torch.Tensor(ss_norm)        
        ########## descriptors normlization ##########
...
def merge_dataset_objs(dataset_1, dataset_2):
    ...
    new_dataset = MoleculeDataset(root='dataset_conf/chembl_with_labels',
...
def _load_esol_dataset(input_path):
    ...
    assert len(smiles_list) == len(labels)
    return smiles_list, rdkit_mol_objs_list, labels.values
# input_path = 'dataset_conf/esol/raw/delaney-processed.csv'
...
# root_path = 'dataset_conf/chembl_with_labels'
...
def create_all_datasets():
    ...
    for dataset_name in downstream_dir:
        print(dataset_name)
        root = "dataset_conf/" + dataset_name
        os.makedirs(root + "/processed", exist_ok=True)
        dataset = MoleculeDataset(root, dataset=dataset_name)
        print(dataset)


    dataset = MoleculeDataset(root = "dataset_conf/chembl_filtered", dataset="chembl_filtered")
    print(dataset)
    dataset = MoleculeDataset(root = "dataset_conf/zinc_standard_agent", dataset="zinc_standard_agent")
    print(dataset)
