...
def pretrain_random_split(dataset, task_idx=None, null_value=0,
                          frac_train=0.8, frac_valid=0.1, seed=0, smiles_list=None):
    """
    :param dataset:
    :param task_idx:
    :param null_value:
    ...
    train_idx = all_idx[:int(frac_train * num_mols)]
    valid_idx = all_idx[int(frac_train * num_mols):]

    assert len(set(train_idx).intersection(set(valid_idx))) == 0
    assert len(train_idx) + len(valid_idx) == num_mols
